...
local redzlib = loadstring(game:HttpGet("https://raw.githubusercontent.com/saurce-caude/u/main/try"))()
...
Tab.info:AddTextBox({
  ...
  Tab.info:AddTextBox({
    ...
    local function invokeServerWithArgs(item, number)
      local args = {
          [1] = "AddPoint",
          [2] = item,
          [3] = number
  ...
  local function contains(name, val)
    for i = 1, #name do
        if name[i] == val then
            return true
        end
